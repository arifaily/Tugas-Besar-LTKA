...
def rule_based_allocate(user: UserRequest) -> dict:
    ...
    prb = max(min_prb, int(base_prb * congestion * queue_factor * cap_factor))

    lat_req     = user.latency_req
    est_latency = lat_req * (1.5 - prb / 80) + (user.prb_usage / 100) * lat_req * 0.3
    est_latency = round(max(1.0, est_latency), 2)
        ...
        try:
            ...
            if user.prb_cap and user.prb_cap < 1.0:
                result["prb_allocated"] = max(
                    5, int(result["prb_allocated"] * user.prb_cap))
