...
USER_PROFILES = {
    ...
    "industry":  {"bw_base": 2.0, "lat_req": 50,  "priority": 2, "prb_base": 25},
    "normal":    {"bw_base": 0.5, "lat_req": 200, "priority": 1, "prb_base": 10},
}
...
def compute_expected_latency(prb_allocated: int, latency_req: int,
                              prb_usage: float, queue_len: int) -> float:
    """
    Estimasi latency berdasarkan PRB yang dialokasikan.
    Makin banyak PRB → makin rendah latency.
    """
    # Base: latency berkurang seiring PRB naik
    prb_ratio       = prb_allocated / 80.0
    base_latency    = latency_req * (1.5 - prb_ratio)
    congestion_add  = (prb_usage / 100) * latency_req * 0.3
    queue_add       = (queue_len / 40)  * latency_req * 0.2
    noise           = np.random.uniform(-5, 5)

    return round(max(1.0, base_latency + congestion_add + queue_add + noise), 2)
...
def generate_dataset(n_samples: int = 5000) -> pd.DataFrame:
    ...
    for user_type in user_types:
        ...
        expected_latency = compute_expected_latency(
            prb_opt, profile["lat_req"], prb_usage, queue_len)
